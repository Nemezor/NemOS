...
void gdt_setup_global_descriptor_table() {
	GDT = (long long*)memory_kalloc(GDT_ENTRIES * sizeof(long long));

	GDT[0] = gdt_create_descriptor_entry(0, 0, 0);
	GDT[1] = gdt_create_descriptor_entry(0, 0xFFFFF, GDT_CODE_PL0);
	GDT[2] = gdt_create_descriptor_entry(0, 0xFFFFF, GDT_DATA_PL0);
	GDT[3] = gdt_create_descriptor_entry(0, 0xFFFFF, GDT_CODE_PL3);
	GDT[4] = gdt_create_descriptor_entry(0, 0xFFFFF, GDT_DATA_PL3);

	__asm_utils_load_gdt((long)GDT, GDT_ENTRIES * sizeof(long long) - 1);
}
